...
describe Terrminology do
  ...
  example do
    ...
    sys.concepts(vs.identifier, code: 'active').first.should == sys.concepts(vs.identifier).find{|c| c.code == 'active'}
    sys.concepts(vs.identity,   code: 'active').first.should == sys.concepts(vs.identity).find{|c| c.code == 'active'}

    sys.concepts(vs.identifier,    code: 'reactivated', display: 'Reactivated', definition: 'Reactivated').should be_empty
    concept = sys.add_concept(vs.identifier, code: 'reactivated', display: 'Reactivated', definition: 'Reactivated')
    sys.concepts(vs.identifier,    code: 'reactivated', display: 'Reactivated', definition: 'Reactivated').should_not be_empty
